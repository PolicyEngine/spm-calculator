...
3. Keep version 1.0.0 marked preview until that exact package is published. Only after publication, regenerate with `SPM_PUBLISHED_PACKAGE_VERSION=1.0.0`, rebuild using the intended `NEXT_PUBLIC_BASE_PATH`, and perform the root-owned deployment/live checks.
